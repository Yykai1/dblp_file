...
def geturl(url):
    ...
    # 这边可以增加一个控制条件，只有结果是精准匹配（Exact matches）的话才进行操作
    # for judge in soup.find_all('div', attrs={'class': 'body hide-body'}):
    # 获取判断的内容
    for jud in soup.find_all('div', attrs={'class': 'body hide-body'}):
        i = i + 1
        if i == 2:
            # print(judge)
            title = jud.select('p')
            judge = title[0].get_text()
            print(judge)
            break
    # 判断条件（只做一次精确查找，没有的话文本文件里面就不会有输出结果）
    if judge == 'Exact matches':
        # 如果是满足要求，获取url
        for item in soup.find('ul', attrs={'class': 'result-list'}):
            # print(item)
            thing = item.select('a')
            # print(thing)
        ...
        print('There is no exact matches')
...
def openfile(list):
    with open('teacher.txt', 'r', encoding='gbk') as f:
        list_temp = []
